...
public class ConfiguracioAutenticacio {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.authorizeHttpRequests (
                (requests) -> requests
                        .requestMatchers("/listado", "/eventos", "/stats", "fragments", "/pago", "/userDetails/", "/editar", "/perfil", "/register", "/makeRegistration", "/login", "/error403", "/").permitAll() //Permitir acceso a todos
                        .requestMatchers("/bloquejats", "/desbloqueja/{id}", "/deleteUser", "/editUser", "/cambiarPermiso", "/assignRole").hasRole("admin") //Permitir acceso solo a ADMIN
