...
public class ConfiguracioAutenticacio {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.authorizeHttpRequests (
                (requests) -> requests
                        .requestMatchers("/stats", "fragments", "/pago", "/editar", "/perfil", "/register", "/makeRegistration", "/login", "/error403", "/").permitAll() //Permitir acceso a todos
                        .requestMatchers("/eventos", "/nuevoEvento", "/userDetails/", "/cambiarPermiso", "/listado", "/stats", "/bloquejats", "/desbloqueja/{id}", "/deleteUser", "/editUser", "/assignRole").hasRole("admin") //Permitir acceso solo a ADMIN
